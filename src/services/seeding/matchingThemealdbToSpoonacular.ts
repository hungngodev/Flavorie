...
async function matching() {
    ...
    for (let z = currentMatching.progress; z < listOfIngredients.length; z++) {
        ...
        try {
            let { results } = await getAllIngredientsAPI([], [], ingredient.strIngredient.toLowerCase(), 2);
            ...
            if (!Ingredient) {
                ...
                currentMatching.mapIngredients.set(ingredient.strIngredient.toLowerCase().trim(), newIngredientModel._id);
            }
            else {
                console.log("Found in the database");
                currentMatching.mapIngredients.set(ingredient.strIngredient.toLowerCase().trim(), Ingredient._id);
            }
            await currentMatching.save();
        }
        catch (error) {
