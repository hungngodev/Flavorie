...
router.get('/image-fix', async (req, res) => {
    ...
    const newImageLink = await axios.get(`https://api.unsplash.com/search/photos?page=1&query=${name}&client_id=${process.env.UNSPLASH_ACCESS_KEY}`);
    if (type === 'ingredient') {
        await IngredientModel.updateOne({ _id }, { image: newImageLink.data.results[0].urls.small });
    }
