...
import { NotFoundError, ServerError } from "@src/errors/customErrors";
import IngredientModel from "@src/models/IngredientModel";
import MatchingModel from "@src/models/MatchingModel";
import themealData from "@src/services/seeding/themeal-ingredient";
import {
...
async function main() {
  ...
  const catchErrorSeedAPI = async (error: Error): Promise<void> => {
  ...
  async function matching() {
    ...
    for (let z = currentMatching.progress; z < listOfIngredients.length; z++) {
      ...
      console.log(
        z + "." + "  Ingredient To Match: " + ingredient.strIngredient,
      );
      try {
        ...
        console.log("Error occured");
        console.dir(error);
        await catchErrorSeedAPI(error as Error);
      }
