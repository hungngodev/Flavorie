...
export async function modifyOrdinaryInfo(
  ...
  if ("lastName" in reqInfo) {
    user.lastName = reqInfo.lastName;
  }
  if ("city" in reqInfo) {
    user.city = reqInfo.city;
  }
  if ("country" in reqInfo) {
    user.country = reqInfo.country;
  }
  if ("description" in reqInfo) {
    user.description = reqInfo.description;
  }

  if ("role" in reqInfo) {
    user.role = reqInfo.role;
  }
  if ("avatar" in reqInfo) {
    user.avatar = reqInfo.avatar;
  }
  if ("avatarPublicId" in reqInfo) {
    user.avatarPublicId = reqInfo.avatarPublicId;
  }
  if ("preferences" in reqInfo) {
    user.preferences = reqInfo.preferences;
  }
  if ("allergy" in reqInfo) {
    user.allergy = reqInfo.allergy;
  }
  if ("diet" in reqInfo) {
    user.diet = reqInfo.diet;
  }
  await user.save();
}
