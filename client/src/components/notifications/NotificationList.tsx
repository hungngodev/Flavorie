...
import { Box, Heading, IconButton, Link, Text, VStack } from '@chakra-ui/react';
                        ...
                        >
                            <Box flex="1" textAlign="left">
                                <Link as={RouterLink} onClick={() => handleClick(noti)}>
                                    <Text fontWeight={noti.status ? 'normal' : 'bold'}>{noti.message.title}</Text>
                                    <Text>{formatDistanceToNow(new Date(noti.timestamp), { addSuffix: true })}</Text>
                                </Link>
                            </Box>
