  ...
  return (
      <Link to={onClickF ? '' : link ?? ''}>
          ...
          >
              ...
              {!expanded && (
                  <motion.div
                      className={` capitalize invisible absolute left-full z-50  ml-1
              w-min -translate-x-3 text-nowrap rounded-md  text-secondary-foreground opacity-20 transition-all 
              group-hover:visible group-hover:translate-x-0 group-hover:opacity-100 text-gray-600`}
                  >
                      {text}
                  </motion.div>
              )}
  ...
  return (
      <SidebarContext.Provider value={{ expanded }}>
          <div className={`space-between bg relative z-[100] flex h-full w-min flex-col  border-r shadow-sm`}>
              <button onClick={setExpanded} className="cursor-pointer rounded-lg p-1.5 hover:bg-violet-100 ">
                  <ChevronFirst
                      className={`${expanded ? '' : 'rotate-180'} ml-4 stroke-gray-600 transition-all duration-500 `}
                  />
              </button>
