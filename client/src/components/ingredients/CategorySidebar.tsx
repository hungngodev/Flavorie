    ...
    return (
        <Link to={onClickF ? '' : link ?? ''}>
            <motion.li
                ...
                key={index}
                className={`group relative my-1 flex w-min cursor-pointer items-center  rounded-md px-3 py-2 font-medium capitalize  transition-colors ${active ? 'bg-violet-200' : 'bg-background text-secondary-foreground  hover:bg-violet-100'}`}
                onClick={onClickF}
            >
                {icon}
                <motion.span
