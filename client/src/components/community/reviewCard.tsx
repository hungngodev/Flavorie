...
interface ReviewCardProps {
    review: Review;
    onReply: (id: string, postId: string, content: string, username: string, src: string, parentReviewId: string) => void;
...
const ReviewCard: React.FC<ReviewCardProps> = ({ review, onReply, onEdit, onDelete }) => {
    ...
    const handleReply = async (content: string, parentReviewId: string) => {
        try {
            ...
            onReply(review.id, review.postId, response, review.author.username, review.author.src, parentReviewId);
            setReply(false);
        } catch (error) {
    ...
    return (
        ...
        <HStack align="start" mb="4" spacing="2">
            ...
            {reply && (
                <Box mt="2">
                    <ReviewForm postId={review.postId} userId={review.userId} avatar={review.author} onSubmit={handleReply} parentReviewId={review.id} />
                </Box>
            )}
            </VStack>
        </HStack>
