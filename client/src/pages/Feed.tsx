...
const Feed = () => {
    ...
    const { data, status, isError, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteQuery({
        queryKey: ['newsfeed'],
        queryFn: fetchFeed,
        initialPageParam: 1,
    ...
    useEffect(() => {
        const updateFeed = async () => {
            console.log(data);
            if (status === 'success' && data && !isFetchingNextPage) {
                const postload = data?.pages.flatMap((page) => parsePost(page.data));
                await dispatch(getFeed({ posts: postload }));
            }
        };
        updateFeed();
    }, [data, fetchNextPage, status, fetchNextPage, dispatch, isFetchingNextPage]);
